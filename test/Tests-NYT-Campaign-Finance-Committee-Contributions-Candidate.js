QUnit.module("CandidateNew");

/*
 * CANDIDATE LEADERS
 */
QUnit.test( "Creation of a CampaignURL.CandidateNew object", function(assert){	
	var actual_obj = new NYT.CampaignURL.CandidateNew();
	assert.expect(2);
	assert.ok(actual_obj instanceof NYT.CampaignURL);
	assert.ok(actual_obj instanceof NYT.CampaignURL.CandidateNew);
});
...
QUnit.test( "Independent response formats between objects", function(assert){
	var url1 = new NYT.CampaignURL.CandidateNew();
	var url2 = new NYT.CampaignURL.CandidateNew();
