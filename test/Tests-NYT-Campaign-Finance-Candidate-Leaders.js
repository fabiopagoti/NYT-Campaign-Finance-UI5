QUnit.module("CandidateLeaders");

/*
 * CANDIDATE LEADERS
 */
QUnit.test( "Creation of a CampaignURL.CandidateLeaders object", function(assert){	
	var actual_obj = new NYT.CampaignURL.CandidateLeaders();
	assert.expect(2);
	assert.ok(actual_obj instanceof NYT.CampaignURL);
	assert.ok(actual_obj instanceof NYT.CampaignURL.CandidateLeaders);
});

QUnit.test( "Base URL", function(assert){
	var actual_obj = new NYT.CampaignURL.CandidateLeaders();
	var expected = "//api.nytimes.com/svc/elections/us/{version}/finances/{campaign_cycle}/candidates/leaders/{category}.{response_format}?";
...
QUnit.test( "Independent response formats between objects", function(assert){
	var url1 = new NYT.CampaignURL.CandidateLeaders();
	var url2 = new NYT.CampaignURL.CandidateLeaders();
...
QUnit.test( "Generate Callable URL: NO PARAMS", function(assert){
	...
	actual_obj.setCategory(category);
